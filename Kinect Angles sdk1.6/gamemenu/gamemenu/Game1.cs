...
namespace kangles
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        const float timebetweenpics = 5000;
        const float screensavertime = 20000;
        float screensavertimer = screensavertime;
        graphic2d screensavepic = new graphic2d();
        float screensavesize = 1;
        ...
        sprite2d gamelogo1, gamelogo2;
        sprite2d[] winlogo = new sprite2d[7];
        sprite2d gameoverlogo;

        List<graphic2d> greenscreenpics = new List<graphic2d>();
        Boolean drawgreenpic = false;
        int greenpicnum = 0;
        ...
        // Class for drawing angles on the screen
...
//        ImageFrame video = new ImageFrame();

        const int framespersec = 25;
...
//            kinectSensor.ColorStream.Enable(ColorImageFormat.YuvResolution640x480;
//            kinectSensor.ColorStream.Enable(ColorImageFormat.RgbResolution1280x960Fps12);
        ...
        protected override void LoadContent()
        {
            ...
            camerasound = Content.Load<SoundEffect>("camera1"); // Loads the sound effect
            ...
            background = new graphic2d(Content, "newbackground2", displaywidth, displayheight);
            ...
            gamelogo1 = new sprite2d(Content, "kangleslogo", displaywidth / 2, displayheight - 100, 1.1f, Color.White, true);
            gamelogo1.rect.Y = gamelogo1.rect.Height / 2;
            gamelogo2 = new sprite2d(Content, "kangleslogo", displaywidth / 2, displayheight - 100, 0.4f, Color.White, true);
            gamelogo2.rect.Y = displayheight - gamelogo2.rect.Height / 2 - 10;

            winlogo[0] = new sprite2d(Content, "p1wins", displaywidth / 2, displayheight - 100, 1.1f, Color.White, true);
            winlogo[0].rect.Y = winlogo[0].rect.Height / 2;
            winlogo[1] = new sprite2d(Content, "p2wins", displaywidth / 2, displayheight - 100, 1.1f, Color.White, true);
            winlogo[1].rect.Y = winlogo[1].rect.Height / 2;
            winlogo[2] = new sprite2d(Content, "drawlogo", displaywidth / 2, displayheight - 100, 1.1f, Color.White, true);
            winlogo[2].rect.Y = winlogo[2].rect.Height / 2;
            winlogo[3] = new sprite2d(Content, "bonus", displaywidth / 4, 100, 0.8f, Color.White, true);
            winlogo[4] = new sprite2d(Content, "bonus", (displaywidth * 3) / 4, 100, 0.8f, Color.White, true);
            winlogo[5] = new sprite2d(Content, "p1newhigh", displaywidth / 2, 150, 0.8f, Color.White, true);
            winlogo[6] = new sprite2d(Content, "p2newhigh", displaywidth / 2, 340, 0.8f, Color.White, true);

            gameoverlogo = new sprite2d(Content, "gameover", displaywidth / 2, displayheight - 100, 1f, Color.White, true);
            ...
            typeoptions[1, 0] = new sprite2d(Content, "percentages1", 220, 140, 1, Color.White, true);
            typeoptions[1, 1] = new sprite2d(Content, "percentages2", 220, 140, 1, Color.Yellow, true);
            typeoptions[2, 0] = new sprite2d(Content, "decimal1", 340, 140, 1, Color.White, true);
            typeoptions[2, 1] = new sprite2d(Content, "decimal2", 340, 140, 1, Color.White, true);
            typeoptions[3, 0] = new sprite2d(Content, "fractions1", 460, 140, 1, Color.White, true);
            typeoptions[3, 1] = new sprite2d(Content, "fractions2", 460, 140, 1, Color.Yellow, true);
            typeoptions[4, 0] = new sprite2d(Content, "bearings1", 580, 140, 1, Color.White, true);
            typeoptions[4, 1] = new sprite2d(Content, "bearings2", 580, 140, 1, Color.Yellow, true);
            typeoptions[5, 0] = new sprite2d(Content, "all1", 700, 140, 1, Color.White, true);
            typeoptions[5, 1] = new sprite2d(Content, "all2", 700, 140, 1, Color.Yellow, true);
            ...
            //kinectSensor.NuiCamera.ElevationAngle = cameraangle;
...
//                kinectSensor.DepthStream.Range = DepthRange.Near;
        ...
        void resetround(float gtime)
        {
            ...
            if (gamemode == numberoftypes - 1)
                questiontype = randomiser.Next(numberoftypes - 1);
            else
                questiontype = gamemode;

            if (questiontype == 0)
            {
                angletoguess = makeangles.generateangle(skilllevel, out round2nearest);
                specialcharacter = '~';
            }
            else if (questiontype == 1)
            {
                angletoguess = makeangles.generatepercentage(skilllevel, out round2nearest);
                specialcharacter = '%';
            }
            else if (questiontype == 2)
            {
                angletoguess = makeangles.generatedecimal(skilllevel, out round2nearest);
                specialcharacter = ' ';
            }
            else if (questiontype == 3)
            {
                makeangles.generatefraction(out numer, out denom, skilllevel, out round2nearest);
                angletoguess = (int)(Math.Round(((float)numer / (float)denom) * 100f, 0));
                specialcharacter = '%';
            }
            else if (questiontype == 4)
            {
                bearingtype = makeangles.generatebearing(skilllevel, out round2nearest, out angletoguess);
                specialcharacter = '~';
            }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            mousepointer1.oldposition = mousepointer1.position;
        ...
        public void updatemenu(float gtime)
        {
            ...
            if (mousepointer1.position == mousepointer1.oldposition)
            {
                if (screensavertimer >= 0)
                {
                    screensavertimer -= gtime;
                    if (picnames.Count() > 0)
                    {
                        screensavepic.rect.X++;
                        screensavepic.rect.Y++;
                        screensavesize -= 0.002f;
                        screensavepic.rect.Width = (int)((float)screensavepic.image.Width * screensavesize);
                        screensavepic.rect.Height = (int)((float)screensavepic.image.Height * screensavesize);
                    }
                }
            }
            else
            {
                screensavertimer = screensavertime;
            }
            ...
            if (screensavertimer < 0)
            {
                ...
                if (picnames.Count() > 0)
                {
                    string filename = Content.RootDirectory + "\\..\\pictures\\" + picnames[randomiser.Next(picnames.Count())];
                    screensavertimer = timebetweenpics;
                    if (File.Exists(@filename))
                        screensavepic = new graphic2d(graphics, Content, filename, displaywidth, displayheight);
                }
                else
                    screensavertimer = screensavertime;
            }
        ...
        public void drawmenu()
        {
            ...
            gamelogo1.drawme(ref spriteBatch);
            // Draw menu options
            for (int i = 0; i < numberofoptions; i++)
            {
                if (optionselected == i)
            ...
            spriteBatch.DrawString(fontwhite, "Programming and Design: David Renton ", new Vector2(280, displayheight - 50), Color.White, 0, new Vector2(0, 0), 0.5f, SpriteEffects.None, 0);
            spriteBatch.DrawString(fontwhite, "Twitter @drenton72  Email drenton@reidkerr.ac.uk", new Vector2(250, displayheight - 33), Color.White, 0, new Vector2(0, 0), 0.5f, SpriteEffects.None, 0);
            spriteBatch.DrawString(fontwhite, "Facebook: www.facebook.com/TheKTeamPIL", new Vector2(270, displayheight - 16), Color.White, 0, new Vector2(0, 0), 0.5f, SpriteEffects.None, 0);

            if (screensavertimer < timebetweenpics && (picnames.Count() > 0))
            {
                screensavepic.drawme(ref spriteBatch);
            }
            ...
            switch (ingamestate)
            {
                case 1:
                    ...
                    if (countdown < 0)
                    {
                        ...
                        ingamestate = 2;
                        resetround(gtime);
                        if (saveallpics && round > questions2ask) savepictures();
                        drawgreenpic = false;
                    }
                    else
                    {
                        if (keys.IsKeyDown(Keys.F) && !drawgreenpic && greenscreenpics.Count() > 0)
                        {
                            drawgreenpic = true;
                            greenpicnum = randomiser.Next(greenscreenpics.Count());
                        }
                    }
                ...
                case 2:
                    ...
                    using (body = kinectSensor.SkeletonStream.OpenNextFrame(0))
                    {
                        if (body != null)
                        {
                            ...
                            {
                                angles[i] = (int)(Math.Round((float)angles[i] / (float)round2nearest, 0) * round2nearest);

                                if (angles[i] < 1) angles[i] = 360;
                    ...
                    {
                        // Take Picture in higher quality
                        ...
                        */
                        ...
                        for (int i = 0; i < numberofplayers; i++)
                        {
                            ...
                            {
                                gamer[i].score += makeangles.angle2percentage(360 - Math.Abs((int)angletoguess - (int)gamer[i].angle));
                            }
                            else if (questiontype == 1 || questiontype == 3)
                            {
                                gamer[i].angle = makeangles.angle2percentage((int)gamer[i].angle);
        ...
        int entername(int namenumber)
        {
            if (namenumber >= 0)
            {
                if (keyboardreleased && keys.IsKeyDown(Keys.Enter))
                {
                    if (namenumber == 0 && gamer[1].score > highscores[9])
                        namenumber = 1;
                    else
                        namenumber = -1;
                }
                else if (keyboardreleased && keys.IsKeyDown(Keys.Back) && gamer[namenumber].playername.Length > 0)
                {
                    // Allow for backspace
                    gamer[namenumber].playername = gamer[namenumber].playername.Substring(0, gamer[namenumber].playername.Length - 1);
                }
                else
                {
                    // Enter new High Score Name
                    char nextchar = sfunctions.getnextkey();
                    char lastchar = '!';
                    if (gamer[namenumber].playername.Length > 0)
                        lastchar = Convert.ToChar(gamer[namenumber].playername.Substring(gamer[namenumber].playername.Length - 1, 1));
                    if (nextchar != '!' && (nextchar != lastchar || keycounter < 0))
                    {
                        keycounter = 200;
                        gamer[namenumber].playername += nextchar;
                        if (gamer[namenumber].playername.Length > 15)
                            gamer[namenumber].playername = gamer[namenumber].playername.Substring(0, 15);
                    }
                }
            }
            return namenumber;
        ...
        public void drawgame()
        {
            ...
            switch (ingamestate)
            {
                case 1:
                    ...
                    if (angletoguess > 0)
                    {
                        ...
                        {
                            drawangles(makeangles.percentage2angle((int)angletoguess), makeangles.percentage2angle((int)gamer[0].angle), makeangles.percentage2angle((int)gamer[1].angle));
                        }
                        else if (questiontype == 2)
                        {
                            drawangles(makeangles.decimal2angle(angletoguess), makeangles.decimal2angle(gamer[0].angle), makeangles.decimal2angle(gamer[1].angle));
                        }
                        ...
                        for (int i = 0; i < numberofplayers; i++)
                            if (gamer[i].angle == angletoguess)
                                if (i == 0)
                                    winlogo[3].drawme(ref spriteBatch);
                                ...
                                //                                      Color.PaleVioletRed, MathHelper.ToRadians(0), new Vector2(0, 0), 2f, SpriteEffects.None, 0);
                                    ...
                                    winlogo[4].drawme(ref spriteBatch);
                    ...
                    if (drawgreenpic)
                    {
                        greenscreenpics[greenpicnum].rect.Height = (int)((float)displayheight / 3f);
                        greenscreenpics[greenpicnum].rect.Width = (int)((float)greenscreenpics[greenpicnum].image.Width * ((float)greenscreenpics[greenpicnum].rect.Height / (float)greenscreenpics[greenpicnum].image.Height));
                        greenscreenpics[greenpicnum].rect.X = displaywidth - greenscreenpics[greenpicnum].rect.Width;
                        greenscreenpics[greenpicnum].rect.Y = displayheight - greenscreenpics[greenpicnum].rect.Height;

                        greenscreenpics[greenpicnum].drawme(ref spriteBatch);
                    }
                ...
                case 2:
                    ...
                    {
                        spriteBatch.DrawString(mainfont, bearingtype, new Vector2(displaywidth / 2 - 63, 8),
                            Color.Aquamarine, MathHelper.ToRadians(0), new Vector2(0, 0), 1.8f, SpriteEffects.None, 0);
                    }
                    // Draw angles
                ...
                default:
                    ...
                    {
                        if (numberofplayers == 2)
                            winlogo[0].drawme(ref spriteBatch);
                    }
                    else
                    {
                        if (gamer[0].score < gamer[1].score)
                            winlogo[1].drawme(ref spriteBatch);
                        else
                            winlogo[2].drawme(ref spriteBatch);
                    }

                    if (keyfocus == 0)
                    {
                        winlogo[5].drawme(ref spriteBatch);
                    ...
                    {
                        winlogo[6].drawme(ref spriteBatch);
                        spriteBatch.DrawString(mainfont, gamer[1].playername, new Vector2(60, 375),
                                Color.LightBlue, MathHelper.ToRadians(0), new Vector2(0, 0), 2f, SpriteEffects.None, 0);
                    }

                    gameoverlogo.drawme(ref spriteBatch);
            ...
            if (countdown > 0)
                ...
                gamelogo2.drawme(ref spriteBatch);
        ...
        // Load the list of picture filenames into list array
        void loadfilenames()
        {
            DirectoryInfo dir = new DirectoryInfo(Content.RootDirectory + "\\..\\pictures");
        ...
        void loadfunnypics()
        {
            DirectoryInfo dir = new DirectoryInfo(Content.RootDirectory + "\\..\\funnies");
            if (dir.Exists)
            {
                FileInfo[] files = dir.GetFiles("*.png");
                foreach (FileInfo file in files)
                {
                    string filename = Content.RootDirectory + "\\..\\funnies\\" + file.Name;
                    greenscreenpics.Add(new graphic2d(graphics, Content, filename, displaywidth, displayheight));
                }
            }
        }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            switch (gamestate)
            {
                ...
                case 99:
                    spriteBatch.Begin();
                    spriteBatch.DrawString(fontwhite, "Error please connect Kinect to this PC", new Vector2(20, 100),
                        Color.Black, MathHelper.ToRadians(0), new Vector2(0, 0), 1.5f, SpriteEffects.None, 0);
                    spriteBatch.DrawString(fontwhite, "Also ensure Kinect SDK v1.5 is installed", new Vector2(20, 200),
                        Color.Black, MathHelper.ToRadians(0), new Vector2(0, 0), 1.5f, SpriteEffects.None, 0);
                    spriteBatch.End();
            ...
            //    spriteBatch.Begin();
            //  spriteBatch.DrawString(mainfont, "time between updates " + ((float)gameTime.ElapsedGameTime.TotalMilliseconds).ToString(), new Vector2(60, 70),
            //    Color.PaleVioletRed, MathHelper.ToRadians(0), new Vector2(0, 0), 0.5f, SpriteEffects.None, 0);
            // spriteBatch.End();
